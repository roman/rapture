...
let
  ...
  parsePlugins = builtins.concatMap (
    p:
    if p ? rapturePluginInputs then
      [ p ]
    else
      builtins.warn "Skipping plugin ${p.name}. It must be built using rapture.mkPlugin" [ ]
  );

  getPluginName = plugin: (assertPluginType plugin).name;

  getPluginDependencies = plugin: map getPluginName plugin.rapturePluginInputs or [ ];

  getPluginsFromScope =
    pluginsScope:
    lib.removeAttrs pluginsScope [
      "callPackage"
      "newScope"
      "overrideScope"
      "packages"
  ...
  concatPluginEmacsInputs =
    plugins: epkgs: builtins.concatLists (map (plugin: plugin.emacsInputs epkgs) plugins);

  # concatPluginOverride combines the overrides of each plugin.
  concatPluginOverride =
    plugins: self: super:
    builtins.foldl' lib.mergeAttrs { } (map (plugin: plugin.pluginOverride self super) plugins);

  concatPluginBuildInputs = plugins: builtins.concatLists (map (plugin: plugin.buildInputs) plugins);

  concatPluginRuntimeInputs =
    plugins: builtins.concatLists (map (plugin: plugin.runtimeInputs) plugins);

  concatPluginFontPackages =
    plugins: builtins.concatLists (map (plugin: plugin.fontPackages) plugins);
  ...
  mkPlugin =
    {
      ...
      runtimeInputs ? [ ],
      # Font packages needed by the plugin. Home Manager can install these into
      # the platform font registry while buildEmacs keeps them in the closure.
      fontPackages ? [ ],
      emacsInputs ? (_epkgs: [ ]),
  ...
  buildConfig =
    ...
    let
      ...
      # for mkPlugin calls.
      pluginScope = lib.makeScope lib.callPackageWith (
        self:
      ...
      fontPackages = concatPluginFontPackages sortedPlugins;
...
in
{
  inherit
    ...
    mkPlugin
    ;
  concatSortedPluginContents = concatPluginContents;
}
